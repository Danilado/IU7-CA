class Spline:
    ...
    def getcCoeffs(self, start, end):
        ...
        for i in range(2, len(self.mat)):
            ...
            tetaarr[i] = theta(dy, dy1, h_i, h_i1,
                               tetaarr[i - 1], ksiarr[i - 1])
    ...
    def getdCoeffs(self, start, end):
        ...
        for i in range(len(self.mat) - 2):
            d[i] = (self.c[i + 1] - self.c[i]) / \
                (3 * (self.mat[i + 1][0] - self.mat[i][0]))

        d[-1] = ((end / 2) - self.c[-1]) / \
            (3 * (self.mat[-1][0] - self.mat[-2][0]))
        return d
    ...
    def getbCoeffs(self, start, end):
        ...
        for i in range(len(self.mat) - 2):
            ...
            b[i] = (self.mat[i + 1][1] - self.mat[i][1]) / h_i - \
                1 / 3 * h_i * (self.c[i + 1] + 2 * self.c[i])
        h_i = (self.mat[-1][0] - self.mat[-2][0])
        b[-1] = (self.mat[-1][1] - self.mat[-2][1]) / h_i - \
            1 / 3 * h_i * ((end / 2) + 2 * self.c[-1])
...
def apprspline3d(mat, x, y):
    ...
    for yi in range(len(mat.y)):
        sp = Spline(mat.get_zx_from_y(yi), 0, 0)
